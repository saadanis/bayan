...
class qMatcherAnnotater():
    ...
    def __init__(self, path):
        suras = buildSuraIndex(path+'dfiles/quran-index.xml')
        self.all = {}
        self.qOrig = buildVerseDics(suras)
        self.qNorm =  buildVerseDics(suras)
        self.stops =  loadStops(path+'dfiles/nonTerminals.txt')
        self.ambig = set([])
        self.minLen = 3                                   #minimum acceptable match length 
        addAyat(path+'dfiles/quran-simple.txt', suras, self.all,self.qOrig,self.qNorm, self.ambig, self.minLen,self.stops)
